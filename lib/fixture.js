...
(function() {
  ...
  Fixture = function() {
    ...
    this._location = null;
    this._datacenter = [];
    return this._superDirty = true;
  };

  Fixture.prototype.wiretap = function(collection) {
    var methods;
  ...
  Fixture.prototype.loadFixturesIntoMemory = function(force) {
    if (force == null) {
      force = false;
    }
  ...
  Fixture.prototype.findFixtures = function(absFixturePath, extension) {
    var files;
  ...
  Fixture.prototype.setupConnection = function(connection, load) {
    if (load == null) {
      load = true;
    }
  ...
  Fixture.prototype.resetConnections = function() {
    return this.connections = [];
  };

  Fixture.prototype.ensureConnectionsReady = function(done) {
  ...
  Fixture.prototype.ensureCollectionsExistInConnection = function(done) {
    var collectionNames;
  ...
  Fixture.prototype.insertAllDataIntoDatabase = function(done) {
    var collectionNames;
    collectionNames = Object.keys(this.collections);
    return async.each(this.connections, (function(_this) {
      return function(conn, nextConn) {
  ...
  Fixture.prototype.destroyAllDataFromDatabase = function(done) {
    var collectionNames;
    collectionNames = Object.keys(this.collections);
    this._superDirty = true;
    return async.each(this.connections, (function(_this) {
  ...
  Fixture.prototype.commenceMassSurveillance = function(done) {
    return async.each(this.connections, (function(_this) {
      return function(conn, nextConn) {
  ...
  Fixture.prototype.ready = function(done) {
    return async.series([
      (function(_this) {
  ...
  Fixture.prototype.destroyDatacenter = function(done) {
  ...
  Fixture.prototype.unDirtyifyCollection = function(collection, done) {
    var docs;
  ...
  Fixture.prototype.unremoveDocById = function(findparam, collection, done) {
  ...
  Fixture.prototype.restoreRemove = function(dataitem, done) {
  ...
  Fixture.prototype.uninsertDoc = function(findparam, collection, done) {
    return collection.secureChannel.remove({
      _id: findparam._id
    }, done);
  };

  Fixture.prototype.restoreInsert = function(dataitem, done) {
  ...
  Fixture.prototype.unupdateDocById = function(findparam, collection, done) {
    var collectionName, doc, id;
    id = String(findparam._id);
    collectionName = collection.name;
    doc = this.documents[collectionName][id];
  ...
  Fixture.prototype.restoreUpdate = function(dataitem, done) {
    var collection, findparams, reinserts;
    collection = dataitem.collection;
    findparams = dataitem.args["0"];
    reinserts = null;
  ...
  Fixture.prototype.restoreFindAndModify = function(dataitem, done) {
  ...
  Fixture.prototype.selectivelyRestoreDatabase = function(done) {
    return async.whilst((function(_this) {
      ...
      return function(next) {
        ...
        switch (dataitem.method) {
          case "insert":
            return _this.restoreInsert(dataitem, next);
          case "update":
            return _this.restoreUpdate(dataitem, next);
          case "remove":
            return _this.restoreRemove(dataitem, next);
          case "findAndModify":
            return _this.restoreFindAndModify(dataitem, next);
        }
      };
    })(this), done);
  };

  Fixture.prototype.populate = function(done) {
    if (this._superDirty) {
      return async.series([
        (function(_this) {
          return function(next) {
  ...
  Fixture.prototype.initModels = function(done) {
    return this.ready(done);
  };

  Fixture.prototype.use = function(ignore) {};
